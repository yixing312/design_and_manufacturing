...
bolt_pretension_ids = [197, 199, 201, 203, 205, 207]
file_path = "E:/code/ws_python/design_and_manufacturing-main/data"
# TODO 该路径与py端不一致
...
while countdown > 0:
    if os.path.exists(file_path + "/task_queue.txt"):
        if os.path.getsize(file_path + "/task_queue.txt") == 0:
            break
        with open(file_path + "/task_queue.txt", "r", encoding="utf8") as f:
            # 如果文件为空，退出循环
